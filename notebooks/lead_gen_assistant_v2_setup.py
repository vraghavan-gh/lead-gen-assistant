...
rows = [
    Row(rep_id="REP001", first_name="Alex",   last_name="Chen",    email="alex.chen@company.com",    team="enterprise_sales", territory=["US","CA"], industries=["Technology","Financial Services"], is_active=True, current_load=12, max_load=50, created_at=now, updated_at=now),
    Row(rep_id="REP002", first_name="Maria",  last_name="Santos",  email="maria.santos@company.com", team="enterprise_sales", territory=["US","GB"], industries=["Healthcare","Manufacturing"],       is_active=True, current_load=8,  max_load=50, created_at=now, updated_at=now),
    Row(rep_id="REP003", first_name="James",  last_name="Wright",  email="james.wright@company.com", team="mid_market_sales", territory=["US"],      industries=["Technology","Retail"],              is_active=True, current_load=15, max_load=60, created_at=now, updated_at=now),
    Row(rep_id="REP004", first_name="Priya",  last_name="Sharma",  email="priya.sharma@company.com", team="mid_market_sales", territory=["US","IN"], industries=["Financial Services","Technology"],  is_active=True, current_load=10, max_load=60, created_at=now, updated_at=now),
    Row(rep_id="REP005", first_name="Marcus", last_name="Johnson", email="marcus.j@company.com",     team="smb_sales",        territory=["US"],      industries=["All"],                              is_active=True, current_load=5,  max_load=80, created_at=now, updated_at=now),
]

spark.createDataFrame(rows).write.mode("append").saveAsTable("lead_gen.sales_pipeline.sales_reps")
print(f"✅ Seeded {len(rows)} sales reps")
spark.table("lead_gen.sales_pipeline.sales_reps").select("rep_id","first_name","last_name","team").show()
